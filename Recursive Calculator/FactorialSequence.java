...
public class FactorialSequence extends JFrame implements ActionListener
{
	JPanel panelOne;
	JLabel lblFactorial,  lblN, lblSum, lblSequence,lblIcon;
	...
	public FactorialSequence()
	{
		...
		lblFactorial = new JLabel("Factorial Sequence");
		lblFactorial.setFont(new Font("Dialog",Font.BOLD,70));
		lblFactorial.setForeground(new Color(50,48,42));
		lblFactorial.setBounds(159,0,840,265);
		...
		panelOne.add(lblFactorial); panelOne.add(lblIcon);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(e.getSource()==btnCompute)
		{
			...
			for(i = 1, sum = 0 ; i <=numberOfValues; i++)
			{
				if(i!=numberOfValues)
				{
				 sequence+=factorialRecursion(i)+",";
				}
				if(i==numberOfValues)
				{
					 sequence+=factorialRecursion(i);
				}
				 sum+=factorialRecursion(i);
			}

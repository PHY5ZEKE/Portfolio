...
public class FibonacciSequence extends JFrame implements ActionListener
{
	JPanel panelOne;
	JLabel lblFibonacci,  lblN, lblSum, lblSequence,lblIcon;
	...
	public FibonacciSequence()
	{
		...
		lblFibonacci = new JLabel("Fibonacci Sequence");
		lblFibonacci.setFont(new Font("Dialog",Font.BOLD,70));
		lblFibonacci.setForeground(new Color(50,48,42));
		lblFibonacci.setBounds(159,0,840,265);
		...
		panelOne.add(lblFibonacci); panelOne.add(lblIcon);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(e.getSource()==btnCompute)
		{
			...
			for(i = 1, sum = 0 ; i <= numberOfValues; i++)
			{
				if(i!=numberOfValues)
				{
				 sequence+=fibonacciRecursion(i)+",";
				}
				if(i==numberOfValues)
				{
					 sequence+=fibonacciRecursion(i);
				}
				 sum+=fibonacciRecursion(i);
			}

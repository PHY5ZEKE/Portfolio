...
public class SquareSequence extends JFrame implements ActionListener
{
	JPanel panelOne;
	JLabel lblSquare,  lblN, lblSum, lblSequence,lblIcon;
	...
	public SquareSequence()
	{
		...
		lblSquare = new JLabel("Square Sequence");
		lblSquare.setFont(new Font("Dialog",Font.BOLD,70));
		lblSquare.setForeground(new Color(50,48,42));
		lblSquare.setBounds(159,0,840,265);
		...
		panelOne.add(lblSquare); panelOne.add(lblIcon);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(e.getSource()==btnCompute)
		{
			...
			for(i = 1, sum = 0 ; i <=numberOfValues; i++)
			{
				if(i!=numberOfValues)
				{
				 sequence+=squareRecursion(i)+",";
				}
				if(i==numberOfValues)
				{
					 sequence+=squareRecursion(i);
				}
				 sum+=squareRecursion(i);
			}
	...
	public static int squareRecursion(int n)
	{
		...
		{
		return squareRecursion(n-1) + 2*n-1;
		}
	}
